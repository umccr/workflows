...
if [ ! -z "$S3_WGS_INPUT_DIR" ]; then
    ...
    if [ ! -z "$(aws s3 ls s3://umccr-primary-data-prod/Blombery-Diagnostic/SBJ00287/WGS/2020-03-04/umccrised/SBJ00287__SBJ00287_MDX200040_L2000113/purple/ | grep purple.cnv.gene.tsv | awk '{print $4}')" ]; then
        PURPLE=$(aws s3 ls s3://umccr-primary-data-prod/Blombery-Diagnostic/SBJ00287/WGS/2020-03-04/umccrised/SBJ00287__SBJ00287_MDX200040_L2000113/purple/ | grep purple.cnv.gene.tsv | awk '{print $4}')
    else
        PURPLE=$(aws s3 ls s3://umccr-primary-data-prod/Blombery-Diagnostic/SBJ00287/WGS/2020-03-04/umccrised/SBJ00287__SBJ00287_MDX200040_L2000113/purple/ | grep purple.gene.cnv | awk '{print $4}')
    fi
...
echo "PUSH results"
aws s3 sync --only-show-errors ${job_output_dir} s3://${S3_RESULT_BUCKET}/${S3_OUTPUT_PATH}/RNAsum
